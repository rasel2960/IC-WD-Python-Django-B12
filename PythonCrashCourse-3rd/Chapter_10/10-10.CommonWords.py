...
try:
    path = Path('files/a_room_with_a_view.txt')
    contents = path.read_text(encoding='utf-8')

    if 'the' in contents.lower():
    ...
    print(f"\nThe word 'the' appears {the_count} times in the file {path.name}.")
    print(f"\nthe word 'the ' appears {the_space_count} times in the file {path.name}.")